...
// val conf = new SparkConf().setAppName("NoiseCount").setMaster("local")
...
val request = sc.textFile("loudacre/request_tonow")
val noiserequest = request.filter(line => line.split(",")(5).contains("Noise"))
// val rawgeodata = noiserequest.map(line => (getgeodata(line, 50),
// getgeodata(line, 51)))
val rawgeodata = noiserequest.map(line => (getgeodata(line, 38), getgeodata(line, 39), getgeodata(line, 1)))
val temp = rawgeodata.filter(line => line._1 != "" && line._2 != "" && (line._3(9) == '7' || line._3(9) == '8'))
val cleangeodata = temp.map(line => (line._1, line._2))
...
val result = noise_insp_rdd.map(getDistanceFromLatLonInKm).filter(line=>line(1).toDouble < 0.5).groupBy(line=>line(0)).map(line=>(line._1, line._2.size))
